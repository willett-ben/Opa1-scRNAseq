...
#         Supp - GSEA of hallmark pathways on clusters 1 and 3
...
top20perCluster <- allMarkers %>% group_by(cluster) %>% top_n(20, avg_log2FC)
```
####    DotPlots of top defining genes
```{r include = TRUE, echo = TRUE}

DotPlot(object = Tcell_data_object, features = unique(top10perCluster$gene)) +
         theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
DoHeatmap(Tcell_data_object, features = top20perCluster$gene, group.by = "seurat_clusters") + scale_fill_gradient2(high = "yellow1", mid ="#009966", low = "navy")
