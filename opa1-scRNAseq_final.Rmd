...
#USER INPUT REQUIRED - update seruat object (2/2)
filtered_data_object <- dataset
...
#rank by log2FC
cluster1_ranks <- data.frame(gene = cluster1_data$gene,
                             log2FC = cluster1_data$avg_log2FC)
...
cluster1_fgsea_results <- fgsea(pathways = gmt_hallmark_list,
                                stats = ranks_1,
                                minSize = 15,
                                maxSize = 500)
...
cluster3_ranks <- data.frame(gene = cluster3_data$gene,
                             log2FC = cluster3_data$avg_log2FC)
ranks_3 <- cluster3_ranks$log2FC
names(ranks_3) <- cluster3_ranks$gene

cluster3_fgsea_results <- fgsea(pathways = gmt_hallmark_list,
                                stats = ranks_3,
                                minSize = 15,
                                maxSize = 500)
...
Hallmark_Myc_targets_v1 <- mapIds(org.Mm.eg.db,
                                  Mm.h_entrez$HALLMARK_MYC_TARGETS_V1,
                                  'SYMBOL',
                                  'ENTREZID')
Hallmark_Myc_targets_v2 <- mapIds(org.Mm.eg.db,
                                  Mm.h_entrez$HALLMARK_MYC_TARGETS_V2,
                                  'SYMBOL',
                                  'ENTREZID')
myc_combined <- c(Hallmark_Myc_targets_v1, Hallmark_Myc_targets_v2)
  
Tcell_data_object <- AddModuleScore(Tcell_data_object,
                                    features = list(myc_combined),
                                    name = 'myc_combined')
...
writeMM(counts, paste0(filtered_dir,
                       matrix_file))

# Output cell barcodes
write.table(as.data.frame(barcodes), paste0(filtered_dir, barcodes_file),
           col.names = FALSE, row.names = FALSE, sep = "\t")

# Output feature names
features <- data.frame("gene_id" = gene_names,"gene_name" = gene_names,type = "Gene Expression")
write.table(as.data.frame(features), sep = "\t", paste0(filtered_dir, genes_file),
