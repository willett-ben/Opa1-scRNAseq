...
```{r include=TRUE, echo=TRUE}
#Save/load if needed
#saveRDS(object = Tcell_data_object, "~/Documents/Sandbox/Opa1-scRNAseq/seurat_objects/Tcell_data_obj_forAnalysis")
Tcell_data_object <- readRDS("~/Documents/Sandbox/Opa1-scRNAseq/seurat_objects/Tcell_data_obj_forAnalysis.RDS")
```
...
system(command = "cnmf prepare --output-dir ~/Documents/Sandbox/Opa1-scRNAseq/cnmf/output/ --name opa1_cnmf -c ~/Documents/Sandbox/Opa1-scRNAseq/cnmf/matrix.mtx -k 4 5 6 --max-nmf-iter 2000 --n-iter 100 --numgenes 2000")

system(command = "cnmf factorize --output-dir ~/Documents/Sandbox/Opa1-scRNAseq/cnmf/output --name opa1_cnmf --worker-index 0 --total-workers 1")

system(command = "cnmf combine --output-dir ~/Documents/Sandbox/Opa1-scRNAseq/cnmf/output --name opa1_cnmf")


system(command = "cnmf k_selection_plot --output-dir ~/Documents/Sandbox/Opa1-scRNAseq/cnmf/output --name opa1_cnmf")

system(command = "cnmf consensus --output-dir ~/Documents/Sandbox/Opa1-scRNAseq/cnmf/output --name opa1_cnmf --components 5 --local-density-threshold 0.01 --show-clustering")
...
usage_file <- read.table("~/Documents/Sandbox/Opa1-scRNAseq/cnmf/output/opa1_cnmf/opa1_cnmf.usages.k_5.dt_0_01.consensus.txt", sep = '\t', row.names = 1, header = TRUE)

spectra_score <- read.table("~/Documents/Sandbox/Opa1-scRNAseq/cnmf/output/opa1_cnmf/opa1_cnmf.gene_spectra_score.k_5.dt_0_01.txt", sep = '\t', row.names = 1, header = TRUE)
  
spectra_tpm <- read.table("~/Documents/Sandbox/Opa1-scRNAseq/cnmf/output/opa1_cnmf/opa1_cnmf.gene_spectra_tpm.k_5.dt_0_01.txt", sep = '\t', row.names = 1, header = TRUE)
...
Mm.h <- readRDS(file="~/Documents/Sandbox/MSigDB/Mm.h.all.v7.1.entrez.rds")
...
Mm.c5_gobp <- gmtPathways("~/Documents/Sandbox/MSigDB/m5.go.bp.v2023.1.Mm.entrez.gmt")
...
# Starting on pathway anslysis
```{r load pathways}
#These were downloaded from online. Presumably from https://bioinf.wehi.edu.au/MSigDB/v7.1/ (need to double check that with Scott)



Mm.h <- readRDS(file="~/Documents/Sandbox/MSigDB/Mm.h.all.v7.1.entrez.rds")
Mm.c2 <-readRDS(file = "~/Documents/Sandbox/MSigDB/Mm.c2.all.v7.1.entrez.rds")
Mm.c5bp<-readRDS(file = "~/Documents/Sandbox/MSigDB/Mm.c5.bp.v7.1.entrez.rds")
Mm.c7<-readRDS(file = "~/Documents/Sandbox/MSigDB/Mm.c7.all.v7.1.entrez.rds")

tester_df <- msigdbr(species = "mouse", category = "C5", subcategory = "BP") %>% dplyr::select(gs_name, entrez_gene)
head(tester)




Hallmark_Heme_metabolism <- mapIds(org.Mm.eg.db, Mm.h$HALLMARK_HEME_METABOLISM, 'SYMBOL', 'ENTREZID')

```

# Check if top spectra scores are the same as GEP_defining_genes
```{r load pathways}
